import { euclidean } from './utils/eucliudean';
import getDistanceMatrix from './utils/Matrix';
...
export function agnes(data, options = {}) {
  ...
  let distanceMatrix = new Matrix(data.map(item => item.map(items => items.value)));
  ...
  for (let n = 0; n < numLeaves - 1; n++) {
    const [row, column, distance,smallestName] = getSmallestDistance(distanceMatrix,data);
